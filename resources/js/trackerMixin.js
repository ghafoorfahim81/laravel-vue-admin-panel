...
export default {
    ...
    methods: {
        ...
        deleteFromAttachmentArray(index) {
            // Remove the attachment from the array based on index
        ...
        updateCheckedStatus(recordId) {
            axios.patch(`/tracker/updateIsChecked/${recordId}`)
                .then(response => {
                    const updatedRecord = response.data.tracker;

                    if (updatedRecord) {
                        const index = this.apiData.data.findIndex(record => record.id === updatedRecord.id);
                        if (index !== -1) {
                            this.$set(this.apiData.data, index, updatedRecord);
                        }
                    } else {
                ...
                axios.post('/trackers/store', formData, {
                    ...
                    if (response.status == 200) {
                        ...
                        if(!this.isShowPage){
                            ...
                            console.log('this is response', response.tracker)
        ...
        editRecord(id = null, index) {
            ...
            axios.get(url).then((res) => {
                ...
                this.trackerForm.out_date = response.out_date;
